...
function navigationStressActions() {
  const pattern = [
  ...
  for (let cycle = 1; cycle <= 4; cycle += 1) {
    ...
    actions.push(action(`cycle-${cycle}-navigate-auxiliary`, 'navigate', 'success', [0, 1], { label: cycle % 2 === 1 ? '及时率' : '测试人员' }, 5_000));
  }
  return actions;
}

function selectionStressActions() {
  const actions = [];
...
export const MISUSE_WORKFLOWS = freeze([
  {
  ...
  },
  {
    ...
    actions: [...navigationStressActions(), action('restart', 'restart', 'success', [2, 3], {}, 30_000)]
  },
  {
